...
def detect_duplicates():
    output = []
    output.append("--- DUPLICATE SCHEDULES AUDIT ---")
    ...
    for s in schedules:
        ...
        if key in seen:
            output.append(f"[DUPLICATE FOUND] ID: {s.id} | USER: {key[0]} | POLICY: {s.policy.policy_number} | START: {s.start_date} | END: {s.end_date}")
    ...
    if pol:
        ...
        for s in ss:
            ...
            output.append(f"ID: {s.id} | CERT: {cert} | PERIOD: {s.start_date} -> {s.end_date} | GROOSS: {s.gross_premium}")

    with open(r'd:\insurance_claim_system_NEW\tmp\duplicate_results.txt', 'w', encoding='utf-8') as f:
        f.write('\n'.join(output))
